...
public class ProductController implements ActionListener {
    ...
    public ProductController(ProductView productView, DataAccess dataAdapter) {
    ...
    private void saveProduct() {
        ...
        int productExpiration;
        try {
            productExpiration = Integer.parseInt(productView.getTxtProductExpiration().getText());
        }
        catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(null, "Invalid product Expiration! Please provide a valid expiration date!");
            return;
        }


        String productVendor = productView.getTxtProductVendor().getText().trim();

        if (productVendor.length() == 0) {
            JOptionPane.showMessageDialog(null, "Invalid product vendor name! Please provide a non-empty vendor name!");
            return;
        }
        ...
        product.setExpiration(productExpiration);
